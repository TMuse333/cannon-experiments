...
const sphere = createSphere(world, scene,camera,re);
...
const objectsToInteract = [sphere.mesh,box.mesh];
const cleanupRaycaster = createRaycaster(camera, scene, renderer, objectsToInteract, (clickedObject) => {
  ...
  if (clickedObject === sphere.mesh) {
    // Apply an impulse to the sphere in the y-direction
    sphere.body.applyImpulse(new CANNON.Vec3(0, 5, 0), sphere.body.position);
  }
...
// To clean up the event listener when you're done, call the cleanup function
// cleanupRaycaster();
