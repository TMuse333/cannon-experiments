...
export function createTree(world, scene) {
  ...
  const trunkBody = new CANNON.Body({ mass: 1, shape: trunkShape });
  ...
  for (let i = 0; i < 4; i++) {
    ...
    branchMesh.name = `branch${i}`; // Assign a name to the branch mesh
    scene.add(branchMesh);
  }

  function updateTree(positionOffset) {
    trunkMesh.position.copy(trunkBody.position).add(positionOffset);
    trunkMesh.quaternion.copy(trunkBody.quaternion);

    branchBodies.forEach((branchBody, index) => {
      const branchMesh = scene.getObjectByName(`branch${index}`);
      if (branchMesh) {
        branchMesh.position.copy(branchBody.position).add(positionOffset);
