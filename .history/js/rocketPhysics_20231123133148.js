...
export class RocketPhysics {
  ...
  onBeginContact(event) {
    ...
    if (
      (contact.bi === this.cannonBody && contact.bj.material === this.groundRocketContactMaterial) ||
      (contact.bi.material === this.groundRocketContactMaterial && contact.bj === this.cannonBody)
    ) {
      console.log('Rocket hit the ground.');
      this.resetFlightTime(); // Reset the flight time when the rocket hits the ground
    }
  }
